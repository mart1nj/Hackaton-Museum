...
namespace Open.Tests.Domain.Common {
    [TestClass] public class MetricTests : EntityBaseTests<Metric<CurrencyData>, CurrencyData> {
        class testClass : Metric<CurrencyData> {
            public testClass(CurrencyData dbRecord) : base(dbRecord) { }
        }
        protected override Metric<CurrencyData> getRandomObject() {
            createdWithNullArg = new testClass(null);
            dbRecordType = typeof(MetricData);
        ...
        {
            Assert.IsFalse(Metric<CurrencyData>.IsFormula(null));
            Assert.IsFalse(Metric<CurrencyData>.IsFormula(string.Empty));
            var s = GetRandom.String();
            var i = GetRandom.Int32(0, s.Length);
            Assert.IsFalse(Metric<CurrencyData>.IsFormula(s));
            Assert.IsTrue(Metric<CurrencyData>.IsFormula(s.Insert(i, "^")));
        }
